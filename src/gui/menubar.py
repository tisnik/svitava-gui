...
class Menubar(tkinter.Menu):
    ...
    def __init__(self, parent, main_window, icons, configuration: Configuration):
        ...
        self.renderer_menu.add_command(
            label="New fractal",
        ...
        self.processor_menu.add_command(
            label="New filter",
        ...
        self.compositor_menu.add_command(
        ...
        self.palette_menu.add_command(
            label="Palette editor", image=icons.edit_icon, compound="left", underline=0
        )
        ...
        self.add_cascade(
            label="File", menu=self.file_menu, underline=0, font=main_menu_font
        )
        self.add_cascade(
            label="Renderer", menu=self.renderer_menu, underline=0, font=main_menu_font
        )
        self.add_cascade(
            label="Processor",
            menu=self.processor_menu,
            underline=0,
            font=main_menu_font,
        )
        self.add_cascade(
            label="Compositor",
            menu=self.compositor_menu,
            underline=0,
            font=main_menu_font,
        )
        self.add_cascade(
            label="Palette", menu=self.palette_menu, underline=1, font=main_menu_font
        )
        self.add_cascade(
            label="Settings", menu=self.settings_menu, underline=0, font=main_menu_font
        )
        self.add_cascade(
            label="Help", menu=self.help_menu, underline=0, font=main_menu_font
        )

        self.parent.bind("<F1>", lambda event: help())
        self.parent.bind("<F11>", lambda event: about())
        self.parent.bind(
            "<Control-n>", lambda event: self.command_fractal_type_dialog()
        )
